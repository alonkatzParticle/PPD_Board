...
export function PlanningPanel({
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900/80 overflow-hidden sticky top-24 animate-fade-in" style={{ height: "calc(100vh - 9rem)" }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <section className="px-4 py-4">
          ...
          {plannedTasks.length === 0 ? (
            <div className="mt-4 py-10 flex flex-col items-center gap-2 text-zinc-700">
              <ClipboardList className="w-8 h-8" />
              <p className="text-sm font-medium text-zinc-600">No draft tasks yet</p>
              <p className="text-xs text-zinc-700 text-center">
                Use the form below to plan who makes what this week
              </p>
            </div>
          ) : (
            <div className="mt-2.5 space-y-0.5">
      ...
      {/* ── Add task form / DB warning ────────────────────────────────────── */}
      <div className="flex-shrink-0 border-t border-zinc-800">
        {dbError ? (
          <div className="p-4 flex items-center justify-center gap-2 text-xs text-amber-400/80">
            <span>⚠</span>
            <span>Database not connected — planned tasks cannot be saved</span>
          </div>
        ) : (
          <AddTaskForm onAdd={(name, assignee) => onAddTask(product, name, assignee)} />
        )}
      </div>
    </div>
  );
...
function AddTaskForm({ onAdd }: { onAdd: (name: string, assignee: string | null) => Promise<void> }) {
